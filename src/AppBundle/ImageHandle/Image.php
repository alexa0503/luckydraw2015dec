...
class Image
{
    ...
    public function create()
    {
        ...
        if ($size->getHeight() > 600 || $size->getWidth() > 600) {
            $image->thumbnail(new Box(600, 600), ImageInterface::THUMBNAIL_INSET)->save($this->file_path . '/' . $this->file_name);
        }
        $image = $imagine->open($image_path);
        #生成缩略图
    ...
    {
        //var_dump($file->getErrorMessage(), $file->getMaxFilesize());
        if ($file->getError() != 0 || null == $file->guessExtension() || !in_array($file->guessExtension(), array('png', 'gif', 'jpeg', 'png'))) {
            return false;
        } else {
